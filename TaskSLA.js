...
TaskSLA.prototype = {
	...
	_updatePauseDuration: function(/* glide_date_time */ pausedAt) {
		...
		//** Author : Fayçal EL GHARBI **//
		//** Date : 25/07/2017 **//
		//** Description : Don't take the last pause time into account after 'in observation' if 'Callback' field is true **//
		
				if (this.taskSLAgr.task.u_boolean_1==true){  // If the 'Callback' field on the incident form is true, don't do anything
				}
				else{
					this.taskSLAgr.pause_duration.setDateNumericValue(this._calculatePD(pausedAt)); // Add the pause duration
				}
						
		if (!this.taskSLAgr.schedule){  // If we have a Schedule
			if (this.taskSLAgr.task.u_boolean_1==true){  // If the 'Callback' field on the incident form is true, don't do anything
			}
			else{
			this.taskSLAgr.business_pause_duration = this.taskSLAgr.pause_duration; // Add the pause duration with the Schedule
			}
		}
			else{
				if (this.taskSLAgr.task.u_boolean_1==true){  // If the 'Callback' field on the incident form is true, don't do anything
					...
					this.taskSLAgr.business_pause_duration.setDateNumericValue(this._calculateBPD(pausedAt)); // Add the pause duration without the Schedule
				}
				}
	},
	...
	_insertBreachTrigger: function() {
		...
		if (!this.taskSLAgr.active || this.breachedFlag || !this.breachTimerEnabled)
			return;
